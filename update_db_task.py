...
# command line with args for celery
...
def update_db():
    ...
    for item in orders:
        ...
        # todo: create functions for these conditionals - tidy them - possibly shorten the logic
    ...
    # this may be unnecessary - it adds min_sell or max_buy keys so that the sqlite logic doesn't fail
    create_none_val_for_incomplete_data(items)

    # try to write to the local db file item-prices.db
    db_attempt = db_write(items)
    if db_attempt:
        return 'task incomplete'
    return 'task completed'


def create_none_val_for_incomplete_data(items_dct):
    """This simply checks the keys min_sell and max_buy for existence, add key and place None as value if not there"""
    for item in items_dct:
        if items_dct[item].get("min_sell") is None:
            items_dct[item]['min_sell'] = None
        if items_dct[item].get("max_buy") is None:
            items_dct[item]['max_buy'] = None
    return


def db_write(items_dct):
    try:
        sqliteConnection = sqlite3.connect('C:\\Users\\gmbro\\OneDrive\\Desktop\\magnumDongBuyback\\db\\item-prices.db')
        ...
        cur.execute("CREATE TABLE IF NOT EXISTS ITEM_PRICES("
                    ...
                    "SELL_PRICE INTEGER"
                    ")")
        for type_id, dct in items_dct.items():
